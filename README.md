...
The **DAILY** shortcut sends the exact `Daily briefing` command. That exact command retrieves only the current day's conditions, high, low, and precipitation from Open-Meteo, followed by six recent world stories from The Guardian Open Platform with each article's headline, source, date, and summary. It renders once under **DAILY BRIEFING** directly from verified API fields and never invokes the language model. Under **Settings → Daily Briefing APIs**, configure the location, choose Fahrenheit (the default) or Celsius, and enter the Guardian developer key. Other briefing, headline, or weather phrasings use the configured language model with freshly retrieved Guardian or Open-Meteo data. After a briefing, commands such as `read article 5`, `expand on article 2`, or `summarize article 1` send only the selected article's full Guardian text to the model for a detailed original summary; VIKI reports when that text is unavailable rather than inventing details.
