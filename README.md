...
Dates and times without an explicit timezone are constructed in the browser/device's local timezone rather than UTC. Inputs such as `today @ 5:30pm` and time-only confirmation corrections such as `set it to 5:30pm` use the current local calendar date. Explicit offsets such as `Z`, `-04:00`, or `+01:00` are preserved as absolute instants. A single date response is treated only as the reminder send time and VIKI separately requests the event time; the LLM cannot invent a missing second date. Future-time validation and due-reminder checks use that same browser clock, and a rejected time reports the locally interpreted value for easier correction.
