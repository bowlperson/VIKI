...
The **DAILY** shortcut retrieves current conditions and a four-day forecast from Open-Meteo for the saved location, which defaults to Tucson, Arizona. The weather forecast is relayed first. VIKI then presents six recent world stories from The Guardian Open Platform individually, with each article's headline, source, date, and summary. Configure the location and Guardian developer key under **Settings → Daily Briefing APIs**. Each briefing identifies when its data was retrieved, preserves the verified API details alongside VIKI's summary, and reports weather or news as specifically unavailable when that source fails or returns nothing. Current facts are supplied to the configured language model with web search disabled; VIKI is instructed not to supplement them from memory. The last five conversation turns remain available for follow-up questions about a briefing or a specific supplied article.
