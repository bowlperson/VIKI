...
Settings include optional EmailJS degradation alerts. When enabled and fully configured, VIKI sends a one-time alert when an asset displays exactly 30% remaining and another one-time alert when it displays exactly 15% remaining, provided the latter still has at least three days left. These percentage milestones do not fire merely because an asset is below either percentage. VIKI also continues to send one alert per item/day for assets with fewer than four days remaining, including degraded assets, until the item is removed from inventory. Supply recipient addresses plus an EmailJS service ID, template ID, and public key; the template receives `to_email`, `item_name`, `quantity`, `location`, `days_left`, `percentage_remaining`, `alert_type`, and `message`. EmailJS browser public keys are supported, but Twilio SendGrid secret API keys must remain on a trusted server and must not be placed in this browser application.
