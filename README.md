...
The **DAILY** shortcut sends the exact `Daily briefing` command. That exact command retrieves current conditions and a four-day forecast from Open-Meteo, followed by six recent world stories from The Guardian Open Platform with each article's headline, source, date, and summary. It renders once under **DAILY BRIEFING** directly from verified API fields and never invokes the language model. Configure the location and Guardian developer key under **Settings → Daily Briefing APIs**. Other briefing or headline phrasings use the configured language model with freshly retrieved Guardian data. The full supplied article details are also retained as context so follow-up questions can ask VIKI to expand on a mentioned article without relying on invented facts.
