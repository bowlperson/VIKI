...
The **DAILY** shortcut retrieves current conditions and a four-day forecast from Open-Meteo for the saved location, which defaults to Tucson, Arizona. The weather forecast is relayed first. VIKI then presents six recent world stories from The Guardian Open Platform individually, with each article's headline, source, date, and summary. Configure the location and Guardian developer key under **Settings → Daily Briefing APIs**. The briefing is rendered once under the title **DAILY BRIEFING** directly from the verified API fields, without a separate AI-generated pre-summary. It identifies when its data was retrieved and reports weather or news as specifically unavailable when that source fails or returns nothing. The last five conversation turns remain available for follow-up questions about a briefing or a specific supplied article; the language model is instructed to use only the supplied details for those follow-ups.
